...
class GridMesh:
    ...
    def extract_boundary_conditions(self, bcs_base: BoundariesBase) -> AxesBoundaries:
        ...
        Returns:
            :class:`~pde.grids.boundaries.axes.AxesBoundaries`: Boundary conditions of
            the subgrid
        """
        if not isinstance(bcs_base, AxesBoundaries):
            raise TypeError(
                "Simulations parallelized with MPI only work with boundary conditions "
                "based on the `AxesBoundaries` class, i.e., conditions need to be "

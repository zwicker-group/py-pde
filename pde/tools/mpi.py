...
def make_float_synchronizer(operator: int | str = "MAX") -> Callable[[float], float]:
    """Return function that synchronizes number between multiple processes.

    Args:
        operator (int or str):
            MPI operator to synchronize the numbers. Typical values are "MAX", "MIN",
            or "SUM"

    Returns:
        Function that can be used to synchronize float values across cores
    """
    if parallel_run:
        # in a parallel run, we need to synchronize the float values
        operator_id = Operator.id(operator)

        @register_jitable
        def synchronize_floats(value: float) -> float:
            """Synchronize values accross all cores."""
            return mpi_allreduce(value, operator_id)  # type: ignore

    else:
        # we don't need to do anything in a serial run

        @register_jitable
        def synchronize_floats(value: float) -> float:
            return value

    return synchronize_floats  # type: ignore

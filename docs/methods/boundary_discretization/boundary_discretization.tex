...
A conservative operator can be derived by integrating the definition of the Laplace operator over one spherical shell from $r=r_n - \dr/2$ to $r=r_n  + \dr/2$,
\begin{subequations}
\begin{align}
%	r^2 \partial_\alpha^2 f &= \partial_r\bigl(r^2 \partial_r f(r)\bigr)
%\\
	\int_{r_n - \dr/2}^{r_n + \dr/2} r^2 \partial_\alpha^2 f \diff r
		&= \int_{r_n - \dr/2}^{r_n + \dr/2} \partial_r\bigl(r^2 \partial_r f(r)\bigr) \diff r
\\
	\partial_\alpha^2 f \left[ \frac{r^3}{3} \right]_{r_n - \dr/2}^{r_n + \dr/2}
		&\sim \Bigl[r^2 \partial_r f(r)\Bigr]_{r_n - \dr/2}^{r_n + \dr/2}
\end{align}
\end{subequations}
where we skipped the factors $4\pi$ stemming from the angle integration.
Using $r_n = r_0 + (n + \frac12)\dr$ and introduced the (normalized) shell volumes
\begin{align}
%	V_n &= \frac{\dr^3}{3}\bigl[(n + 1)^3 - n^3\bigr]
	V_n &= \frac{1}{3}\left[
		\left(r_n + \frac{\dr}{2}\right)^3 - 
		\left(r_n - \frac{\dr}{2}\right)^3
	\right]
\notag\\&= \dr \left(	r_n^2 + \frac{\dr^2}{12} \right)
\end{align}
we then arrive at
\begin{align}
	\partial_\alpha^2 f
...
\end{align}
where the derivatives $f'(r)$ are evaluated at the midpoints can can thus be evaluated using
\begin{subequations}
\begin{align}
	f'\left(r_n + \frac{\dr}{2}\right) \sim \frac{y_{n+1} - y_n}{\dr}
\\
	f'\left(r_n - \frac{\dr}{2}\right) \sim \frac{y_n - y_{n-1}}{\dr}
\end{align}
\end{subequations}
In the special case $r_0=0$, we then arrive at
\begin{align}
	\partial_\alpha^2 f

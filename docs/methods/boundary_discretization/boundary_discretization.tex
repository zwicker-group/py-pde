...
We also derive special versions of operators that conserves the total mass.
\end{abstract}
...
%		&\sim \dr^2 \frac{(n + 1)^2 f'\left(r_n + \frac{\dr}{2}\right) - n^2 f'\left(r_n - \frac{\dr}{2}\right)}{V_n}
	...
	\label{eqn:conservative_laplace_spherical_sym}
\end{align}
...
Similarly, we can consider a spherical grid where fields depend on the polar angle~$\theta\in[0,\pi]$, while still keeping azimuthal symmetry, i.e., assuming no dependence on $\phi$.
...
\\
	+  \Delta r
		\bigl[ \sin\theta \partial_\theta  f(r, \theta)\bigr]_{\theta_{m - \frac12}}^{\theta_{m + \frac12}}
\end{multline}
Note that the first term is identical to the term given by \Eqref{eqn:conservative_laplace_spherical_sym} since the angular dependence cancels with the one in $V_{n,m}$.
...
\begin{align}
	\partial_\alpha v_{\alpha} \sim
	 ...
	 \label{eqn:conservative_div_spherically_sym}
\end{align}
...
Similar to the Laplace operator above, we can also derive a conservative divergence operator for spherical grids with a dependence on the polar angle $\theta$,
\begin{align}
	\partial_\alpha v_\alpha &= v_r'(r)+\frac{2 v_r(r)}{r} + \frac{v_\theta}{r \tan\theta} + \frac{\partial_\theta v_\theta}{r}
	\;.
\end{align}
The radial part is thus identical to the spherical operator given by \Eqref{eqn:conservative_div_spherically_sym}.
We have so far not analyzed the angular part in detail.

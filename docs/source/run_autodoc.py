...
def main():
    ...
    # Remove the redundant autogenerated package index, which is not included in the
    # custom documentation toctree.
    modules_index = Path(OUTPUT_PATH) / "modules.rst"
    if modules_index.exists():
        logger.info("Remove file `%s`", modules_index)
        modules_index.unlink()

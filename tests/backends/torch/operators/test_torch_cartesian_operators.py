...
def test_singular_dimensions_2d(backend, periodic, rng):
    ...
    expected = field.laplace("auto_periodic_neumann", backend=backend).data
    for g in [g2a, g2b]:
        f = ScalarField(g, data=field.data.reshape(g.shape))
        res = f.laplace("auto_periodic_neumann", backend=backend).data.reshape(g1.shape)
...
def test_gradient_1d(backend):
    ...
    res = field.gradient(bcs, backend=backend)
...
def test_2nd_order_bc(backend, rng):
    ...
    field.laplace({"x": {"value": "sin(y)"}, "y": {"value": "x"}}, backend=backend)

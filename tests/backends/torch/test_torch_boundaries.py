...
def test_virtual_points_torch(dim, bc, rng):
    ...
    f2 = field.copy()
    bc_setter = GhostCellSetter(bcs=bcs, dtype=f1.dtype).to(torch_backend.device)
...
def test_inhomogeneous_bcs_1d_torch():
    ...
    setter = GhostCellSetter(bcs=bc, dtype=field.dtype).to(torch_backend.device)
...
def test_inhomogeneous_bcs_2d_torch():
    ...
    setter = GhostCellSetter(bcs=bc, dtype=f2.dtype).to(torch_backend.device)
